...
let galaxy; function createGalaxy() { /* ... unverändert ... */ } createGalaxy();

// === NEU: Planetensystem-Logik ===
const celestialObjects = [];

// Funktion zum Erstellen eines Planeten (oder Schwarzen Lochs)
function createCelestialObject(config) {
    let objectMesh;
    if (config.isBlackHole) {
        // Logik für das realistische Schwarze Loch
        const blackHoleGroup = new THREE.Group();
        const core = new THREE.Mesh(new THREE.SphereGeometry(1.5, 32, 32), new THREE.MeshBasicMaterial({ color: 0x000000 }));
        ...
        objectMesh = blackHoleGroup;
        config.lensingData = { cubeCamera, lensingSphere, core, accretionDisk };
    } else {
        // Logik für einen prozeduralen Planeten
    ...
    label.position.set(0, config.size + 5, 0);
    objectMesh.add(label);

    // Grenzkreis
    const circleGeometry = new THREE.RingGeometry(config.proximityRadius, config.proximityRadius + 0.2, 64);
    const circleMaterial = new THREE.MeshBasicMaterial({ color: 0x00aaff, side: THREE.DoubleSide, transparent: true, opacity: 0.5 });
    ...
    const trailMaterial = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, blending: THREE.AdditiveBlending });
    const trail = new THREE.Line(trailGeometry, trailMaterial);
    scene.add(trail);

    const celestial = {
        mesh: objectMesh, label: label, circle: circle, trail: trail,
        orbitRadius: config.orbitRadius, orbitSpeed: config.orbitSpeed, angle: Math.random() * Math.PI * 2,
        isBlackHole: config.isBlackHole, size: config.size,
        lensingData: config.lensingData
    };
    celestialObjects.push(celestial);
}

// Erstelle die Himmelskörper
createCelestialObject({ name: 'Project_Mariner', isBlackHole: true, size: 1.5, proximityRadius: 12, orbitRadius: 0, orbitSpeed: 0 });
createCelestialObject({ name: 'Project_Mariner (Kopie 2)', color: 0xff8844, size: 2.5, proximityRadius: 15, orbitRadius: 20, orbitSpeed: 0.03 });
createCelestialObject({ name: 'Project_Mariner (Kopie 3)', color: 0x4488ff, size: 3, proximityRadius: 18, orbitRadius: 35, orbitSpeed: 0.02 });
createCelestialObject({ name: 'Project_Mariner (Kopie 4)', color: 0x88ff88, size: 2, proximityRadius: 14, orbitRadius: 50, orbitSpeed: 0.015 });
createCelestialObject({ name: 'Project_Mariner (Kopie 5)', color: 0xff4444, size: 3.5, proximityRadius: 20, orbitRadius: 68, orbitSpeed: 0.01 });
createCelestialObject({ name: 'Project_Mariner (Kopie 6)', color: 0xffff44, size: 2.8, proximityRadius: 17, orbitRadius: 85, orbitSpeed: 0.008 });
createCelestialObject({ name: 'Project_Mariner (Kopie 7)', color: 0x8844ff, size: 2.2, proximityRadius: 16, orbitRadius: 100, orbitSpeed: 0.005 });
...
loader.load(modelURL, (gltf) => {
    ...
    ship = gltf.scene;
    scene.add(ship);
    ship.position.set(0, 0, -50); // Startposition etwas weiter weg
...
function animate() {
    ...
    // Update alle Himmelskörper
    celestialObjects.forEach(obj => {
        // Umlaufbahn
        obj.angle += obj.orbitSpeed * deltaTime;
        obj.mesh.position.x = Math.cos(obj.angle) * obj.orbitRadius;
        obj.mesh.position.z = Math.sin(obj.angle) * obj.orbitRadius;
        
        // Grenzkreis folgt dem Planeten und pulsiert
        obj.circle.position.copy(obj.mesh.position);
        const pulse = Math.sin(elapsedTime * 0.8 + obj.orbitRadius) * 0.5 + 0.5;
        obj.circle.scale.set(1 + pulse * 0.1, 1 + pulse * 0.1, 1);
        obj.circle.material.opacity = 0.3 + pulse * 0.4;

        // Label ausrichten
        if (ship && !isIntroAnimationPlaying) {
            const angle = Math.atan2(ship.position.x - obj.mesh.position.x, ship.position.z - obj.mesh.position.z);
            obj.label.element.style.transform = `rotate(${angle}rad)`;
        }

        // Orbit-Spur aktualisieren
        const trailPositions = obj.trail.geometry.attributes.position;
        const trailColors = obj.trail.geometry.attributes.color;
        const trailLength = 150 * (Math.PI / 180); // 150 Grad
        for (let i = 0; i < 100; i++) {
            const ratio = i / 99;
            const angle = obj.angle - ratio * trailLength;
            trailPositions.setXYZ(i, Math.cos(angle) * obj.orbitRadius, 0, Math.sin(angle) * obj.orbitRadius);
            trailColors.setXYZ(i, 0, 0.67, 1); // Farbe (Blau)
            trailColors.setW(i, 1 - ratio); // Alpha verblasst
        }
        trailPositions.needsUpdate = true;
        trailColors.needsUpdate = true;

        // Proximity-Check für Analyze-Button
        if (ship) {
            const distanceToShip = ship.position.distanceTo(obj.mesh.position);
            const circleCurrentRadius = obj.circle.geometry.parameters.outerRadius * obj.circle.scale.x;
            if (distanceToShip < circleCurrentRadius) {
                isNearAnyPlanet = true;
            }
        }

        // Schwarzes Loch Logik
        if(obj.isBlackHole) {
            obj.lensingData.accretionDisk.rotation.z += 0.005;
            obj.lensingData.lensingSphere.visible = false; obj.lensingData.core.visible = false; obj.lensingData.accretionDisk.visible = false;
            obj.lensingData.cubeCamera.update(renderer, scene);
            obj.lensingData.lensingSphere.visible = true; obj.lensingData.core.visible = true; obj.lensingData.accretionDisk.visible = true;
        }
    });

    // Analyze-Button ein-/ausblenden
    if (isNearAnyPlanet) {
        if (!analyzeButton.classList.contains('ui-visible')) analyzeButton.classList.add('ui-visible');
    } else {
        if (analyzeButton.classList.contains('ui-visible')) analyzeButton.classList.remove('ui-visible');
    }
    ...
    if (ship) {
        ...
        // Kollision nur mit dem ersten Objekt (Schwarzes Loch)
        const blackHoleObj = celestialObjects[0];
        const collisionThreshold = shipRadius + blackHoleObj.size;
        if (ship.position.distanceTo(blackHoleObj.mesh.position) < collisionThreshold) {
    ...
    if (isIntroAnimationPlaying) {
        ...
        if (Math.abs(cameraPivot.rotation.y) < 0.01) { cameraPivot.rotation.y = 0; isIntroAnimationPlaying = false; }
    } else {
        ...
        cameraHolder.rotation.x += cameraVelocity.x; cameraPivot.rotation.y += cameraVelocity.y;
    }
...
// Hilfsfunktionen, die hier zur Vollständigkeit eingefügt werden müssen
