...
loader.load(modelURL, (gltf) => {
  ...
  loadingScreen.addEventListener('click', () => {
    ...
    // Quick Warp erst nach Intro sichtbar (s. animate)
  }, { once: true });
...
// Quick Warp Interaktion (ohne Effekte)
...
function buildWarpList() {
  ...
  const entries = [
    ...
    // (Black Hole weggelassen, da Fokus: Planeten)
  ];
...
/**
 * Springt in den aktuellen Boundary-Kreis des gewählten Planeten:
 * - ermittelt den momentanen Kreisradius (inkl. Puls-Scale)
 * - wählt eine stabile Richtung (vom Zentrum nach außen)
 * - setzt das Schiff innerhalb des Kreises, mit Mindestabstand zum Planeten
 */
function performWarp(targetId) {
  ...
  // Weltposition des Planeten
  const planetPos = new THREE.Vector3();
  p.mesh.getWorldPosition(planetPos);

  // aktueller Boundary-Radius (Geometrie-Radius * Scale)
  const baseR = p.boundaryCircle.geometry.parameters.radius || 0;
  const scale = p.boundaryCircle.scale.x || 1;
  const boundaryR = Math.max(0.1, baseR * scale);

  // Sicherheitsmarge: nicht in den Planeten clippen
  const planetR = p.mesh.geometry.parameters.radius || 1;
  const minDist = planetR + 2.0;

  // Zielabstand: „tief im Kreis“, aber mit Puffer – 75% des Boundary-Kreises
  const targetDist = Math.max(minDist, boundaryR * 0.75);

  // stabile Richtung: vom Zentrum (0,0,0) nach außen zum Planeten
  const dir = planetPos.clone().normalize(); // Unit

  // Zielposition = Planet + dir * targetDist
  const targetPos = planetPos.clone().add(dir.multiplyScalar(targetDist));

  ship.position.copy(targetPos);
  ship.lookAt(planetPos);
  // Kamera wieder hinter das Schiff ausrichten
...
function animate() {
  ...
  if (appState === 'intro') {
    ...
    if (Math.abs(cameraPivot.rotation.y) < 0.01) {
      cameraPivot.rotation.y = 0;
      appState = 'playing';
      quickWarpBtn.classList.remove('hidden'); // JETZT erst sichtbar
    }
