...
let gameState = 'loading'; // Mögliche Zustände: 'loading', 'ready', 'playing'
...
loader.load(modelURL, (gltf) => {
    ...
    // Baue die Hauptszene im Hintergrund
    createGalaxy();
    blackHoleCore = new THREE.Mesh(new THREE.SphereGeometry(1.5, 32, 32), new THREE.MeshBasicMaterial({ color: 0x000000 })); scene.add(blackHoleCore);
    lensingSphere = new THREE.Mesh(new THREE.SphereGeometry(2.5, 64, 64), new THREE.MeshBasicMaterial({ envMap: cubeRenderTarget.texture, refractionRatio: 0.9, color: 0xffffff })); scene.add(lensingSphere);
    accretionDisk = createAccretionDisk();
    const blackHoleLabelDiv = document.createElement('div'); blackHoleLabelDiv.className = 'label'; blackHoleLabelDiv.textContent = 'Project_Mariner'; const lineDiv = document.createElement('div'); lineDiv.className = 'label-line'; blackHoleLabelDiv.appendChild(lineDiv); blackHoleLabel = new CSS2DObject(blackHoleLabelDiv); blackHoleLabel.position.set(0, 7, 0); scene.add(blackHoleLabel);
    pacingCircle = new THREE.Mesh(new THREE.TorusGeometry(12, 0.1, 16, 100), new THREE.MeshBasicMaterial({ color: 0xffffff })); pacingCircle.rotation.x = Math.PI / 2; scene.add(pacingCircle);
    ...
    gameState = 'ready'; // Signalisiere, dass das Spiel bereit ist
...
// KORREKTUR: Der Start-Klick wechselt nur den Zustand
...
function animate() {
    ...
    if (gameState === 'loading') {
        // Zustand 1: Nur Hyperspace animieren
        hyperspace.position.z = (hyperspace.position.z + 2) % 500;
        renderer.render(scene, camera);
        return; // Stoppe hier, bis das Spiel bereit ist
    }
...
// KORREKTUR: Starte den Animations-Loop sofort
animate();
