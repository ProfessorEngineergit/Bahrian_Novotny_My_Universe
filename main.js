...
renderer.domElement.addEventListener('touchstart', (e) => {
    ...
    e.preventDefault();
    for (const touch of e.changedTouches) { if (cameraFingerId === null) { cameraFingerId = touch.identifier; cameraVelocity.set(0, 0); previousTouch.x = touch.clientX; previousTouch.y = touch.clientY; } }
...
renderer.domElement.addEventListener('touchmove', (e) => {
    ...
    for (const touch of e.changedTouches) {
        if (touch.identifier === cameraFingerId) {
            const deltaX = touch.clientX - previousTouch.x; const deltaY = touch.clientY - previousTouch.y;
            cameraVelocity.x += deltaY * 0.0002; cameraVelocity.y -= deltaX * 0.0002;
...
// NEU: Scroll-Rad/Trackpad-Handler für Desktop-Zoom
renderer.domElement.addEventListener('wheel', (e) => {
    e.preventDefault();
    // Normalisiere den Delta-Wert und füge ihn zur Geschwindigkeit hinzu
    const scrollAmount = Math.sign(e.deltaY) * 0.5;
    zoomVelocity += scrollAmount;
}, { passive: false });
...
function animate() {
    ...
    // Bedingung für Return-to-Center prüft jetzt Touch UND Maus
    ...
    // Federung an den Rotationsgrenzen (unverändert)
    ...
    // Geschwindigkeiten anwenden und dämpfen (unverändert)
    ...
    // Harter Stopp für den Zoom (unverändert)
