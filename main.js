...
// NEU: Importiere Post-Processing Pässe
...
// NEU: Post-Processing Setup für den Glow-Effekt
const renderScene = new RenderPass(scene, camera);
const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.4, 0.85);
bloomPass.threshold = 0;
bloomPass.strength = 1.2; // Intensität des Glows
bloomPass.radius = 0.5;   // Radius des Glows
...
// KORREKTUR: Geometrie und Material der Grenzkreise für den Bloom-Effekt
const boundaryGeometry = new THREE.TorusGeometry(1, 0.05, 16, 100);
const boundaryMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff }); // Strahlendes Weiß
const pacingCircle = new THREE.Mesh(new THREE.TorusGeometry(12, 0.1, 16, 100), boundaryMaterial);
...
function createPlanet(data, index) {
    ...
    // KORREKTUR: Erstelle eine Instanz der Torus-Geometrie für den Grenzkreis
    const boundaryRadius = data.radius + 6;
    const boundaryCircle = new THREE.Mesh(new THREE.TorusGeometry(boundaryRadius, 0.1, 16, 100), boundaryMaterial);
    boundaryCircle.rotation.x = Math.PI / 2;
    orbitPivot.add(boundaryCircle); // Heften den Kreis an den Pivot, damit er sich mitbewegt
...
function animate() {
    ...
    if (keyboard['arrowup']) cameraVelocity.x += 0.0005; if (keyboard['arrowdown']) cameraVelocity.x -= 0.0005;
    if (keyboard['arrowleft']) cameraVelocity.y += 0.0005; if (keyboard['arrowright']) cameraVelocity.y -= 0.0005;
    if (keyboard['-']) zoomVelocity += 0.1; if (keyboard['+'] || keyboard['=']) zoomVelocity -= 0.1;
    ...
    if (isIntroAnimationPlaying) {
        ...
        if (ship) {
            ...
            planets.forEach(p => { p.mesh.getWorldPosition(worldPosition); p.labelDiv.style.transform = `rotate(${getAngleToShip(worldPosition)}rad)`; });
        }
...
window.addEventListener('resize', () => {
    ...
    renderer.setSize(window.innerWidth, window.innerHeight);
    composer.setSize(window.innerWidth, window.innerHeight); // Wichtig für den Bloom-Effekt
    labelRenderer.setSize(window.innerWidth, window.innerHeight);
});

// Gekürzte Funktionen zur Übersichtlichkeit, im Code oben aber vollständig enthalten
function createForcefield(radius) { /*...*/ }
function createPlanetTexture(color) { /*...*/ }
function getPinchDistance(e) { /*...*/ }
renderer.domElement.addEventListener('touchstart', (e) => { /*...*/ }, { passive: false });
