...
let forwardFlightTimer = 0;
let isWarping = false;
const NORMAL_SPEED = 0.1;
const WARP_SPEED = 0.8;
let currentMaxSpeed = NORMAL_SPEED;
let currentShipSpeed = 0;
const MAX_BANK_ANGLE = Math.PI / 8;
const NORMAL_FOV = 75;
const WARP_FOV = 110;
...
mapButton.addEventListener('click', () => { 
    alert('Sternenkarte wird noch implementiert!'); 
});
...
nipplejs.create({ 
    ...
    if (data.vector && ship) { 
        joystickMove.forward = data.vector.y; 
        joystickMove.turn = -data.vector.x; 
    } 
}).on('end', () => 
...
function animate() {
    ...
    if (ship) {
        ...
        if (finalForward > 0.1 && Math.abs(finalTurn) < 0.2) {
            forwardFlightTimer += delta;
        } else {
            forwardFlightTimer = 0;
        }
        isWarping = forwardFlightTimer > 3;

        const targetMaxSpeed = isWarping ? WARP_SPEED : NORMAL_SPEED;
        currentMaxSpeed = THREE.MathUtils.lerp(currentMaxSpeed, targetMaxSpeed, 0.03);
        
        const targetFov = isWarping ? WARP_FOV : NORMAL_FOV;
        camera.fov = THREE.MathUtils.lerp(camera.fov, targetFov, 0.03);
        camera.updateProjectionMatrix();

        const targetSpeed = finalForward * currentMaxSpeed;
        currentShipSpeed = THREE.MathUtils.lerp(currentShipSpeed, targetSpeed, 0.05);
        ship.translateZ(currentShipSpeed);

        const targetBankAngle = -finalTurn * MAX_BANK_ANGLE;
        ship.rotation.z = THREE.MathUtils.lerp(ship.rotation.z, targetBankAngle, 0.05);

        if (currentShipSpeed < -0.01) {
