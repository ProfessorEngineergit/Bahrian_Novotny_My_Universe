...
// NEU: Importiere die Post-Processing-Module
...
// === NEU: Post-Processing-Setup ===
const renderScene = new RenderPass(scene, camera);
const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.4, 0.85);
bloomPass.threshold = 0; // Wie hell ein Objekt sein muss, um zu leuchten (0 = alles leuchtet ein bisschen)
bloomPass.strength = 1.0; // Stärke des Leuchteffekts
bloomPass.radius = 0.5; // Radius des Leuchteffekts
...
// === UI Elemente ===
const loadingScreen = document.getElementById('loading-screen'); const progressBar = document.getElementById('progress-bar'); const loadingText = document.getElementById('loading-text'); const infoElement = document.getElementById('info'); const joystickZone = document.getElementById('joystick-zone'); const muteButton = document.getElementById('mute-button'); const analyzeButton = document.getElementById('analyze-button'); const audio = document.getElementById('media-player');

// === Szenerie-Setup ===
scene.add(new THREE.AmbientLight(0xffffff, 0.2)); // Reduzierte Helligkeit, damit der Bloom besser wirkt
const directionalLight = new THREE.DirectionalLight(0xffffff, 0.3);
directionalLight.position.set(10, 20, 15);
scene.add(directionalLight);
let galaxy; function createGalaxy() { /* ... unverändert ... */ } createGalaxy();
const blackHoleCore = new THREE.Mesh(new THREE.SphereGeometry(1.5, 32, 32), new THREE.MeshBasicMaterial({ color: 0x000000 })); scene.add(blackHoleCore);
const cubeRenderTarget = new THREE.WebGLCubeRenderTarget(256, { format: THREE.RGBFormat, generateMipmaps: true, minFilter: THREE.LinearMipmapLinearFilter }); const cubeCamera = new THREE.CubeCamera(1, 1000, cubeRenderTarget); scene.add(cubeCamera);
const lensingSphere = new THREE.Mesh(new THREE.SphereGeometry(2.5, 64, 64), new THREE.MeshBasicMaterial({ envMap: cubeRenderTarget.texture, refractionRatio: 0.9, color: 0xffffff })); scene.add(lensingSphere);
...
// KORREKTUR: Verwende ein helles, weißes Material für die Ringe, damit der Bloom-Effekt greift
const pacingCircleMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, side: THREE.DoubleSide, transparent: true });
const pacingCircle = new THREE.Mesh(new THREE.RingGeometry(12, 12.1, 128), pacingCircleMaterial); // Dünner gemacht
...
function createPlanetTexture(color) { /* ... unverändert ... */ }
function createPlanet(data, index) {
    ...
    // KORREKTUR: Graue Umlaufbahnen werden entfernt
    ...
    const boundaryCircle = new THREE.Mesh(new THREE.RingGeometry(boundaryRadius, boundaryRadius + 0.1, 64), pacingCircleMaterial.clone());
...
const keyboard = {}; let joystickMove = { forward: 0, turn: 0 }; const ROTATION_LIMIT = Math.PI * 0.33; let zoomDistance = 15; const minZoom = 8; const maxZoom = 25; let cameraVelocity = new THREE.Vector2(0, 0); let zoomVelocity = 0; const SPRING_STIFFNESS = 0.03; const DAMPING = 0.90; const LERP_FACTOR = 0.05;
