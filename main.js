...
// === Hyperspace-Setup ===
function createHyperspaceLayer(z) {
    const count = 1500;
    ...
    layer.position.z = z;
    hyperspaceGroup.add(layer);
    return layer;
}
const hyperspace1 = createHyperspaceLayer(0);
const hyperspace2 = createHyperspaceLayer(-1000);

// === Szenerie-Erstellung (wird später aufgerufen) ===
let galaxy; let blackHoleCore; let lensingSphere; let accretionDisk; let blackHoleLabel;
...
function createSceneElements() {
    ...
    mainSceneGroup.add(lensingSphere);
    ...
    blackHoleCore.add(blackHoleLabel);
    pacingCircle = new THREE.Mesh(new THREE.TorusGeometry(12, 0.1, 16, 100), new THREE.MeshBasicMaterial({ color: 0xffffff }));
    pacingCircle.rotation.x = Math.PI / 2;
    mainSceneGroup.add(pacingCircle);
